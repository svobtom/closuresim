...
public class AlgorithmSimpleParallelRunThree implements Runnable {
    ...
    private void testThreeOneDisconnection() {

        synchronized (LOCKER) {

            Set<Disconnection> oneDis = new HashSet<>();

            for (Iterator<Disconnection> it = disconnectionCollector.getDisconnections().iterator(); it.hasNext();) {
                Disconnection dis = it.next();
                if (dis.getNumClosedRoads() == 1) {
                    oneDis.add(dis);
                }
            }

            for (Iterator<Disconnection> it = oneDis.iterator(); it.hasNext();) {
                Disconnection disconnection = it.next();

                for (Iterator<Disconnection> it1 = oneDis.iterator(); it1.hasNext();) {
                    ...
                    for (Iterator<Disconnection> it2 = oneDis.iterator(); it2.hasNext();) {
                        Disconnection disconnection2 = it2.next();

                        if (disconnection2.equals(disconnection) || disconnection2.equals(disconnection1)) {
                            continue;
                        }

                        RoadsSelection rs = new RoadsSelection(3);
                        rs.addRoads(disconnection.getRoads());
                        rs.addRoads(disconnection1.getRoads());
                        rs.addRoads(disconnection2.getRoads());
